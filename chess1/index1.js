...
for(i=0; i<64; i++){
  ...
  boxes.setAttribute("data-cards",`(${x},${y})`)
  // boxes.innerText = "(" +x+ "," +y+ ")";
  if ((x+y) %2 ==0){
    boxes.style.backgroundColor ="darkred"
  }
  else{
    boxes.style.backgroundColor ="goldenrod  "
  }
  x++;
  if(x >8){
    x=1;
...
class Pawn extends piece{
  ...
  getpossibleMoves = () =>{
    ...
    if(this.color == "black"){
      ...
      const box2 = document.querySelector(`div[data-cards="(${this.currentpos.x+1},${this.currentpos.y-1})"]`)
      const box3 = document.querySelector(`div[data-cards="(${this.currentpos.x-1},${this.currentpos.y+1})"]`)
      ...
      if(box3 && box3.dataset.color && box3.dataset.color !=this.color){
      this.moves.push({
        x: this.currentpos.x - 1,
        y: this.currentpos.y + 1
      })
...
class bishop extends piece{
  ...
  getpossibleMoves = () =>{
    ...
    let validartemoves= validatebishopMovesArr(this);
    this.moves=validartemoves
    showmoves(this)
...
const validateRookMovesArr = (piece) => {
  ...
  for (const dir in directions){
    for (const d of directions[dir]){
      const box1  =document.querySelector(`div[data-cards="(${d.x},${d.y})"]`);
